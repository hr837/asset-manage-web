...
<template>
  <div class="component upload-picture flex">
    <el-upload
      ref="uploadRef" v-model:file-list="fileListData" list-type="picture-card" :action="actionUrl"
    ...
    <ul class="upload-items flex flex-wrap">
      <li v-for="(file, index) in fileListData" :key="file.uid" class="upload-item">
        <el-image
          style="width: 146px; height: 146px" :src="file.url" :zoom-rate="1.2" :preview-src-list="imageList"
          :initial-index="index" fit="fill"
        />
        <div class="upload-back">
          <div class="upload-success" />
          <icon-park-outline-check theme="outline" size="24" fill="#fff" />

          <icon-park-outline-close
            class="upload-delete" theme="outline" size="15" fill="#fff"
